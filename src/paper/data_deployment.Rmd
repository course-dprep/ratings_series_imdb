...
data %>% group_by('season_runtime') %>% summarize(average_runtime = mean(series_runtime))
data %>% group_by('rating') %>% summarize(average_rating = mean(rating))

## Regression series runtime and ratings
run_regression_analysis(cleaned_data)

#### Relationship between series runtime and ratings
